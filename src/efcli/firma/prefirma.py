...
from efcli.pdf.pdf_utils import normalizar_pdf

logger = logging.getLogger(__name__)

def prefirmar(lista_pdfs: list, preferencias: dict) -> tuple[list, list] | bool:
    """
        ...
        for i in PARA_ITERAR:
            ...
            with open(f'{i}', 'rb') as f_stream:
                ...
                try:
                    ...
                    normalizado_path, normalizado_stream, lector, escritor = normalizar_pdf(pdf=i, autoconfirmar=preferencias["autoconfirmar_normalizaciones"])
                    i = normalizado_path
    ...
    # esta sección de prefirma requiere su propia lógica de salida extendidad por SIGINT en lugar de usar
    # el wrapper general de core_utils para salida limpia.
    except KeyboardInterrupt:
        print("\nCTRL+C Recibido.")
        if NORMALIZADOS and not preferencias["mantener_normalizados"]:
