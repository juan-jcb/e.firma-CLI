...
from efcli.utils import cripto, general
from efcli.env import config
...
def contexto(firmante_input: dict, pki_ctx: ValidationContext) -> dict | None:
    ...
    print(f"[{Fore.CYAN}INFO{Fore.WHITE}] Definiendo al firmante.")
    ...
    try:
        ...
        print(f'[{Fore.RED}ERROR{Fore.WHITE}] No se puede establecer el contexto PKI del firmante.')
        print('Saliendo...')
        return False
    else:
    ...
    try:
        ...
        print(f"[{Fore.RED}ERROR{Fore.WHITE}] No se pudo instanciar al firmante: {e}")
        return False
    ...
    try:
        ...
        print(f"[{Fore.RED}ERROR{Fore.WHITE}] Error definiendo los meadatos de la firma, no puede firmarse en este estado. {e}")
        return False
    else:
    ...
    if PREFERENCIAS['TST_CMS'] == True:
        try:
            ...
            print(f"[{Fore.LIGHTYELLOW_EX}ADVERTENCIA{Fore.WHITE}] No se pudo configurar timestamp para CMS. Continuando sin el...")
    ...
    if PREFERENCIAS['TST_DSS'] == True:
        try:
            ...
            print(f"[{Fore.LIGHTYELLOW_EX}ADVERTENCIA{Fore.WHITE}] No se pudo configurar timestamp en PDF (/DocTimeStamp). Continuando sin el...")
            ts_dss = None
        else:
    ...
    if CAMPO_VISUAL['usar'] == True:
        ...
        try:
            ...
            print(f"[{Fore.RED}ERROR{Fore.WHITE}] configurando el CAMPO visible de la firma: {e}")
            return False
    else:
    ...
    print(f'\n[{Fore.CYAN}INFO{Fore.WHITE}] Firmante: {x509_utils.leer_subject_simple(cert=X509_SUBJECT)}')
    print(f'[{Fore.CYAN}INFO{Fore.WHITE}] Perfil de firma propuesto: PAdES-B-{''.join(PERFIL_FIRMA_PROPUESTO)}')
    #print(f'\n• Documentos a firmar: {PDFs}')
    #print(pki.leer_ca_chain_simple(chain_path=CA_CHAIN_SUBJECT))
    
    if PREFERENCIAS['OCSP'] == True:
        pass#print(f"\n{MENSAJES_MISC['msj_desfase_temporal']}")
...
def hacer_firma():
    print(f"[{Fore.CYAN}INFO{Fore.WHITE}] Firmando con configuración defecto.")
